...
const char* ssid = <REDEWIFI>;      // Nome da rede WiFi para o ESP fazer a conexão com a internet.Exemplo : "Wificasa", sempre entre aspas duplas.const 
char* password = <PASS>;            // Senha do WiFi. Exemplo: "12345678"
...
String phoneNumber1 = "559588888888";      // Número de telefone para enviar mensagem (Brasil 55) Exemplo : 559599999999 
String apiKey1 = "1234567";                // Chave de API para autenƟcação no CallMeBot, é dada pelo whats CallMeBot após enviar a mensagem : "I allow callmebot to send me messages" 
...
void loop() {
  ...
  if (estado_botao == 0) {                // Se o botão estiver pressionado (estado LOW)
    ...
    if (flag) {                // Verifica se a flag está ativada (evita envio repetido)
      sendMessage(phoneNumber, apiKey, "SOCORRO!!!!, Help me");
