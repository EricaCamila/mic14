...
const char* ssid = "Starlink_CIT";                            // Nome da rede WiFi para o ESP fazer a conexão com a internet.Exemplo : "Wificasa", sempre entre aspas duplas.const 
char* password = "Ufrr@2024Cit";  // Senha do WiFi. Exemplo: "12345678"
...
bool flag = 1; // Variável de controle para evitar múlƟplos envios da mensagem //
...
String apiKey1 = "2597956";                                                              // Chave de API para autenƟcação no CallMeBot, é dada pelo whats CallMeBot após enviar a mensagem : "I allow callmebot to send me messages" 
...
void sendMessage(String phoneNumber, String apiKey, String message) {
  ...
  //String url = "https://api.callmebot.com/whatsapp.php?phone=559591612080&apikey=2597956&text=dfs";
...
void setup() {
  ...
  // Enquanto não esƟver conectado ao WiFi, pisca o LED2
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);                              // Aguarda 500 ms
    Serial.print(".");                       // Imprime um ponto para indicar tentaƟva de conexão
...
void loop() {
  ...
  if (estado_botao == 0) {                // Se o botão esƟver pressionado (estado LOW)
    ...
    if (flag) {                // Verifica se a flag está aƟvada (evita envio repeƟdo)
      ...
      flag = 0;  // DesaƟva a flag após o envio
    }
  } else {
    flag = 1;                 // Se o botão não esƟver pressionado, reseta a flag
